...
function setup_environment {
  bf=""
  n=""
  ORGANISATION="Fundação para a Ciência e a Tecnologia"
  URL="http://www.fct.pt"
...
if ! ask "This installer will only work properly if you are a member of ${bf}Fundação para a Ciência e a Tecnologia.${n}" "Continue" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:npslxfct.fct.pt'})
        server_name = 'npslxfct.fct.pt'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
